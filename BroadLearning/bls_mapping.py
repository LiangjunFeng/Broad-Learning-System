...
class broadnet_mapping:
    ...
    def transform(self,data):
        ...
        inputdata = np.column_stack((mappingdata,enhencedata))
        for elem in self.local_mapgeneratorlist:
            inputdata  = np.column_stack((inputdata , elem.transform(data)))
        return inputdata 
    
    def addingmap_nodes(self, data, label, step = 1, batchsize = 'auto'):
        if batchsize == 'auto':
            batchsize = data.shape[1]
               
        inputdata = self.transform(data)       
        localmap_generator = node_generator()
        extramap_nodes = localmap_generator.generator_nodes(data,step,batchsize,self._map_function)
        
        D = self.pesuedoinverse.dot(extramap_nodes)
        C = extramap_nodes - inputdata.dot(D)
    ...
    def addingmap_predict(self, data, label, step = 1, batchsize = 'auto'):
        self.addingmap_nodes(data, label, step, batchsize)
...
bls = broadnet_mapping(maptimes = 10, 
                       enhencetimes = 10,
                       traintimes = 100,
                       map_function = 'linear',
